...
const Icon = ({ type, className = "w-5 h-5" }) => {
  ...
  // スケッチに近い「重なった四角」のコピーアイコン
  if (type === 'copy') return (
    <svg className={className} fill="none" stroke="currentColor" viewBox="0 0 24 24">
      <path strokeLinecap="round" strokeLinejoin="round" {...style} d="M8 7v8a2 2 0 002 2h6M8 7V5a2 2 0 012-2h4.586a1 1 0 01.707.293l4.414 4.414a1 1 0 01.293.707V15a2 2 0 01-2 2h-2M8 7H6a2 2 0 00-2 2v10a2 2 0 002 2h8a2 2 0 002-2v-2" />
    </svg>
  );
...
export default function MemoApp() {
  ...
  const addMemo = async () => {
    ...
    const { error } = await supabase.from('memos').insert([{ 
      text: writeInput, 
      color: selectedColor.bar, // ここでバーの色を保存
      category: 'normal' 
    }]);
    if (!error) { setWriteInput(""); fetchMemos(); }
  };
          ...
          <div className="animate-in fade-in">
            ...
            <div className="flex gap-2 mb-4">
              {[
                ...
                <button key={index} onClick={() => setSelectedColor(c)} className={`w-10 h-6 rounded-full border ${c.bg} ${c.border} ${selectedColor.bar === c.bar ? 'ring-2 ring-offset-2 ring-gray-100 scale-110' : 'opacity-40'}`} />
              ))}
            </div>
            ...
            <button onClick={addMemo} disabled={!writeInput.trim()} className={`w-full mt-4 py-4 rounded-2xl font-bold transition-all ${writeInput.trim() ? 'bg-emerald-400 text-white shadow-md' : 'bg-gray-100 text-gray-300'}`}>メモを追加する</button>
          </div>
        )}
        ...
        {activeTab === 'timeline' && (
          <div className="space-y-4 animate-in fade-in">
             {normalMemos.map((memo) => {
               ...
               return (
                 <div key={memo.id} className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm relative overflow-hidden flex flex-col">
                    {/* 左端のカラーバー (修正: memo.colorを直接使用) */}
                    ...
                    <div className="flex-grow pr-2">
                      {editId === memo.id ? (
                        ...
                        <div>
                          ...
                          {shouldShowReadMore && (
                            <button onClick={() => toggleExpand(memo.id)} className="text-xs text-emerald-500 font-bold mt-1 hover:opacity-70">
                    ...
                    {/* 下部：日付とアクション (修正: 余白を詰めた mt-2 pt-2) */}
                    <div className="flex justify-between items-end mt-2 pt-2 border-t border-gray-50">
                      <div className="flex items-center gap-2">
                        ...
                        <button onClick={() => copyText(memo.text)} className="text-gray-300 hover:text-emerald-500 transition-colors p-1" title="コピー">
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div className="relative">
                          ...
                          {menuOpenId === memo.id && (
                            <div className="absolute right-0 bottom-8 w-24 bg-white border border-gray-100 rounded-xl shadow-xl z-10 py-1">
