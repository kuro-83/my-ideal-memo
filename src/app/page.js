...
export default function MemoApp() {
  ...
  const [expandedId, setExpandedId] = useState(null); // 続きを読むの管理
      ...
      <header className="pt-8 text-center">
        ...
        <nav className="flex justify-center gap-2 mt-6">
          {['write', 'timeline', 'garden'].map((tab) => (
            <button key={tab} onClick={() => setActiveTab(tab)} className={`p-3 px-8 rounded-t-xl transition-all ${activeTab === tab ? 'bg-[#f2f2f2] text-gray-800' : 'text-gray-200 hover:text-gray-300'}`}>
              <Icon type={tab} />
            </button>
          ))}
        </nav>
      ...
      <main className="max-w-xl mx-auto px-6">
          ...
          <div className="mt-6 animate-in fade-in duration-500">
            ...
            <div className="flex gap-2 mb-4">
              {[{ bg: 'bg-blue-50', border: 'border-blue-200' }, { bg: 'bg-red-50', border: 'border-red-200' }, { bg: 'bg-yellow-50', border: 'border-yellow-200' }, { bg: 'bg-green-50', border: 'border-green-200' }, { bg: 'bg-purple-50', border: 'border-purple-200' }].map((c, index) => (
                <button key={index} onClick={() => setSelectedColor(c)} className={`w-12 h-6 rounded-full border ${c.bg} ${c.border} ${selectedColor.bg === c.bg ? 'ring-2 ring-offset-2 ring-gray-100' : 'opacity-40 border-gray-100'}`} />
              ))}
            </div>

            <div className={`border-2 rounded-2xl p-4 transition-all bg-white ${selectedColor.border}`}>
              <textarea 
                className="w-full h-32 border-none focus:ring-0 outline-none resize-none text-base text-gray-700 placeholder-gray-200 bg-transparent" 
                placeholder="今の気づきを..." 
                value={writeInput} 
            ...
            <button onClick={() => addMemo('normal')} disabled={!writeInput.trim()} className={`w-full mt-4 py-4 rounded-2xl font-bold transition-all ${writeInput.trim() ? 'bg-[#98c9b6] text-white shadow-md' : 'bg-[#f0f7f4] text-[#b8d6ca]'}`}>
              メモを追加する
            </button>
          </div>
        )}
        ...
        {activeTab === 'garden' && (
          <div className="mt-6 animate-in fade-in duration-500">
            ...
            {showSeedForm && (
              <div className="mb-8 bg-[#fcfcfc] border border-gray-200 rounded-2xl p-5 animate-in slide-in-from-top-2">
                <input 
                ...
                <div className="flex justify-end mt-2">
                  <button onClick={() => addMemo('seed')} disabled={!gardenTitle.trim() || !gardenInput.trim()} className="bg-emerald-400 text-white px-6 py-2 rounded-full text-xs font-bold">
                    種をまく
                  </button>
                </div>
              </div>
            ...
            <div className="grid gap-3">
              {gardenMemos.map((memo) => (
                <div key={memo.id} className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm transition-all">
                  <div className="flex justify-between items-center">
                    <h3 className="font-bold text-gray-800">{memo.title}</h3>
                    <button 
                      onClick={() => setExpandedId(expandedId === memo.id ? null : memo.id)}
                      className="text-[10px] font-bold text-emerald-500 bg-emerald-50 px-3 py-1 rounded-full hover:bg-emerald-100 transition-colors"
                    >
                      {expandedId === memo.id ? '閉じる' : '続きを読む'}
                    </button>
                  </div>
                  {expandedId === memo.id && (
        ...
        {activeTab === 'timeline' && (
          <div className="mt-6 space-y-4 animate-in fade-in duration-500">
             {normalMemos.map((memo) => (
               <div key={memo.id} className="bg-white border border-gray-50 rounded-2xl p-5 shadow-sm relative overflow-hidden">
                  <div className={`absolute left-0 top-0 bottom-0 w-1 ${memo.color.replace('-50', '-200')}`} />
